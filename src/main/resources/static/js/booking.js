...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (weekStartParam) {
        const [yy, mm, dd] = weekStartParam.split("-").map(Number);
        if (!Number.isNaN(yy) && !Number.isNaN(mm) && !Number.isNaN(dd)) {
            currentWeekStart = startOfWeek(new Date(yy, mm - 1, dd));
        }
    }
...
// currentWeekStart er allerede mandag i den uge vi viser (lokal tid)
    ...
    try {
        const url = isGuest
            ? `/api/bookings/week?weekStart=${weekStartStr}`
    ...
    const params = new URLSearchParams(window.location.search);
    const openStart = params.get("openStart");
    if (openStart && Array.isArray(allSlots)) {
        const slotToOpen = allSlots.find(s => s && s.startTime === openStart);
        if (slotToOpen) {
            openModal(slotToOpen);
        }
    }
}
...
// Forsøg at læse varighed ud fra teksten "HH:MM-HH:MM"
                ...
                if (!isMemberOpen) {
                    ...
                    const allowedToOpen = isGuest ? true : slot.userAllowed;

                    if (!slot.full && allowedToOpen && !body.classList.contains("past-day")) {
...
// ------------------------------
    function openModal(slot) {
        ...
        if (isGuest) {
            confirmBtn.innerText = "Log ind";
            confirmBtn.onclick = () => redirectToLoginWithReturn(slot);
        } else {
            confirmBtn.innerText = "Book tid";
...
async function confirmBooking() {
    ...
    function redirectToLoginWithReturn(slot) {
        const y = currentWeekStart.getFullYear();
        const m = String(currentWeekStart.getMonth() + 1).padStart(2, "0");
        const d = String(currentWeekStart.getDate()).padStart(2, "0");
        const weekStartStr = `${y}-${m}-${d}`;

        const returnUrl = `/booking?weekStart=${encodeURIComponent(weekStartStr)}&openStart=${encodeURIComponent(slot.startTime)}`;
